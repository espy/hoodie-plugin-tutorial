...
Hoodie, in itself, is basically a small, solid core that handles data storage and sync. Everything else is a modular addition that can be coupled to this core. Out goal is to make Hoodie as extensible as possible while making potential conflicts between the extensions impossible. 
...
And in your backend component, listen for that task appearing and act upon it:
    ...
    hoodie.task.on('change:message', function (db, change) {
...
Now `hoodie.directMessages.add()` actually exists. If you've ever seen the revealing module pattern, you know what this is. 
